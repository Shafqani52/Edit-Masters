...
import { Switch, Table, Flex, Layout, theme, Typography, Button, Modal, Input, Form, Spin, Alert, Select } from 'antd';
...
const Users = () => {
  ...
  const { usersInfo, getAllUsers, handleUserStatus, updateUserRole, deleteMember, error, loading } = usersData();
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <Flex gap="small">
            <Button type="primary" size="medium" onClick={() => viewUserModal(record)}>Edit</Button>
            { record.active ? 
                <Button
                    type="primary"
                    size="medium"
  ...
  return (
    <Flex className="main-content">
      <Flex vertical>
        <NavHeader tab="users" pageTitle="Users" />
      </Flex>
      <Flex flex={1}>
        <Content
          style={{
        ...
        >
          {usersInfo.length ? (
            ...
            <Typography.Title level={3}>No users found!</Typography.Title>
          )}
        </Content>
      </Flex>
        ...
        {/* Edit Modal */}
         <Modal
          className='userModal'
          title={userToView.role === 'admin' ? 'Edit member' : 'Edit user'}
